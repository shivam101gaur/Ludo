...
export const isTokenSafe = (pathPosition: Token['pathPosition']): boolean => {
 ...
 */
export function getTokenGridCoordinates(token: Token): { x: number; y: number } {
  
  // 1. IS IN BASE (pathPosition === -1)
  if (token.pathPosition === -1) {
    const [anchorX, anchorY] = BASE_ANCHORS[token.playerIndex];
  ...
  if (token.pathPosition >= 56) {
    // 8,8 is the absolute mathematical center of a 15x15 grid
    return { x: 8, y: 8 }; 
  }

  // 3. HOME STRETCH (Indices 51 to 55)
  if (token.pathPosition > 50) {
    const step = token.pathPosition - 50; 
    const [x, y] = HOME_STRETCH_GENERATORS[token.playerIndex](step);
  ...
  const mappedIndex = (token.pathPosition + (token.playerIndex * PLAYER_GAP)) % TOTAL_TRACK_STEPS;
